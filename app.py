...
# ---------------------------------------------------------------------
# TEMPORARY — waitlist email flow diagnostic. Deletes any existing waitlist
# row for the given address, submits it fresh, and polls Resend for the
# delivery status of that specific send. Remove this endpoint once the
# flow has been verified — it exists only to confirm delivery without
# passing SUPABASE_SERVICE_KEY / CHKD_RESEND_API_KEY outside of Railway.
# ---------------------------------------------------------------------

@app.post("/chkd/waitlist/test")
async def chkd_waitlist_test(request: Request):
    ...
    if not sb_key:
        raise HTTPException(status_code=503, detail="SUPABASE_SERVICE_KEY not set")
    ...
    if ins_r.status_code != 201:
        raise HTTPException(status_code=502, detail=f"Insert failed: {ins_r.status_code} {ins_r.text[:200]}")

    from services.chkd import CHKD_FROM_EMAIL, _chkd_resend_key
    resend_key = _chkd_resend_key()
    if not resend_key:
        raise HTTPException(status_code=503, detail="CHKD_RESEND_API_KEY not set")
    ...
    send_r = _httpx.post(
        ...
        json={"from": f"Tommy <{CHKD_FROM_EMAIL}>", "to": [email], "subject": subject, "text": body},
        timeout=15,
    )
    if send_r.status_code not in (200, 201):
        raise HTTPException(status_code=502, detail=f"Resend send failed: {send_r.status_code} {send_r.text[:200]}")
    resend_id = send_r.json().get("id")
    ...
    for _ in range(6):
        ...
        if st_r.status_code == 200:
            last_payload = st_r.json()
            status = last_payload.get("last_event")
            if status in ("delivered", "bounced", "complained", "delivery_delayed"):
                break

    return JSONResponse({
        "ok": True,
        "email": email,
        "deleted_existing_row": del_r.status_code in (200, 204),
        "delete_status_code": del_r.status_code,
        "insert_status_code": ins_r.status_code,
        "resend_id": resend_id,
        "resend_last_event": status,
        "resend_payload": last_payload,
    })
